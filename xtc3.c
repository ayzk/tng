...
#define TRESHOLD_INTRA_INTER_DIRECT             \
    1.5 /* How much larger can the direct       \
...
#define TRESHOLD_INTER_INTRA                  \
    5.0 /* How much larger can the intra      \
           frame deltas for the small         \
           triplets be and be accepted anyway \
...
static void insert_value_in_array(unsigned int**     ptr,
                                  int*               nele,
                                  int*               nele_alloc,
...
static void swapdecide(struct xtc3_context* xtc3_context, int* input, int* swapatoms, const int* large_index, int* minint)
{
    int didswap = 0;
    int normal, swapped;
    (void)large_index;
...
#endif
        insert_value_in_array(&xtc3_context->instructions, &xtc3_context->ninstr,
                              &xtc3_context->ninstr_alloc, INSTR_FLIP, "instr");
    }
}
...
#ifdef SHOWIT
        fprintf(stderr, "%6d: %6d %6d %6d\t\t%6d %6d %6d\t\t%6d %6d %6d\n", nencode, input_ptr[nencode],
                input_ptr[nencode + 1], input_ptr[nencode + 2], encode_ints[nencode],
                encode_ints[nencode + 1], encode_ints[nencode + 2], positive_int(encode_ints[nencode]),
...
static void large_instruction_change(struct xtc3_context* xtc3_context, const int i)
{
    ...
    if (xtc3_context->has_large_type[i] != xtc3_context->current_large_type)
    {
        ...
        insert_value_in_array(&xtc3_context->instructions, &xtc3_context->ninstr,
                              &xtc3_context->ninstr_alloc, instr, "instr");
    }
}

static void write_three_large(struct xtc3_context* xtc3_context, const int i)
{
    int m;
    if (xtc3_context->current_large_type == 0)
    {
        for (m = 0; m < 3; m++)
        {
            insert_value_in_array(&xtc3_context->large_direct, &xtc3_context->nlargedir,
    ...
    else if (xtc3_context->current_large_type == 1)
    {
        for (m = 0; m < 3; m++)
        {
            insert_value_in_array(&xtc3_context->large_intra_delta, &xtc3_context->nlargeintra,
    ...
    else
    {
        for (m = 0; m < 3; m++)
        {
            insert_value_in_array(&xtc3_context->large_inter_delta, &xtc3_context->nlargeinter,
...
static void flush_large(struct xtc3_context* xtc3_context, const int n) /* How many to flush. */
{
    int i;
    i = 0;
    while (i < n)
    {
        ...
        for (j = 0;
             (i + j < n) && (xtc3_context->has_large_type[i + j] == xtc3_context->has_large_type[i]); j++)
        {
            ;
        }
        if (j < 3)
        {
            ...
            {
                insert_value_in_array(&xtc3_context->instructions, &xtc3_context->ninstr,
                                      &xtc3_context->ninstr_alloc, INSTR_ONLY_LARGE, "instr");
        ...
        {
            insert_value_in_array(&xtc3_context->instructions, &xtc3_context->ninstr,
                                  &xtc3_context->ninstr_alloc, INSTR_LARGE_RLE, "instr");
            insert_value_in_array(&xtc3_context->rle, &xtc3_context->nrle,
                                  &xtc3_context->nrle_alloc, (unsigned int)j, "rle (large)");
            for (k = 0; k < j; k++)
            {
                write_three_large(xtc3_context, i + k);
            }
...
static double compute_intlen(const unsigned int* ints)
{
    /* The largest value. */
...
static void buffer_large(struct xtc3_context* xtc3_context,
                         int*                 input,
                         const int            inpdata,
                         const int            natoms,
                         const int            intradelta_ok)
{
...
{
    output[(*outdata)++] = (n)&0xFFU;
    output[(*outdata)++] = ((n) >> 8) & 0xFFU;
    output[(*outdata)++] = ((n) >> 16) & 0xFFU;
    output[(*outdata)++] = ((n) >> 24) & 0xFFU;
}
...
static void base_decompress(const unsigned char* input, const int len, unsigned int* output)
{
    unsigned int largeint[MAXMAXBASEVALS + 1];
    unsigned int largeint_tmp[MAXMAXBASEVALS + 1];
    int          ixyz, i, j;
    ...
    {
        fprintf(stderr,
                "Read a larger maxbasevals value from the file than I can handle. Fix"
                " by increasing MAXMAXBASEVALS to at least %d. Although, this is"
                " probably a bug in TRAJNG, since MAXMAXBASEVALS should already be insanely large "
                "enough.\n",
                maxbasevals);
        exit(EXIT_FAILURE);
    }
    ...
    for (ixyz = 0; ixyz < 3; ixyz++)
    {
        ...
        while (nvals_left)
        {
            ...
            if (basegiven == 0)
            {
                base = (unsigned int)(input[0]) | (((unsigned int)(input[1])) << 8)
            ...
            {
                output[outvals + i * 3] =
                        Ptngc_largeint_div(base, largeint, largeint_tmp, maxbasevals + 1);
                for (j = 0; j < maxbasevals + 1; j++)
                {
                    largeint[j] = largeint_tmp[j];
                }
...
static int heuristic_bwlzh(const unsigned int* ints, const int nints)
{
    int i, num;
    num = 0;
    for (i = 0; i < nints; i++)
...
unsigned char* Ptngc_pack_array_xtc3(int* input, int* length, const int natoms, int speed)
{
    ...
    for (j = 0; j < 3; j++)
    {
        fprintf(stderr, "minint[%d]=%d. maxint[%d]=%d large_index[%d]=%d value=%d\n", j,
                xtc3_context.minint[j], j, xtc3_context.maxint[j], j, large_index[j],
                Ptngc_magic(large_index[j]));
    }
#endif
...
#    endif
                ...
                {
                    buffer_large(&xtc3_context, input, inpdata + 3, natoms,
                                 0); /* This is a swapped integer, so inpdata is one atom later and
                ...
                insert_batch(input + inpdata, ntriplets_left, prevcoord, encode_ints, min_runlength,
                             &nencode);
            }
            ...
            )
            {
                ...
                if ((!swapatoms) && (frame > 0))
                {
                    for (i = 0; i < new_runlength; i++)
                    {
                        ...
                        delta[0]  = positive_int(input[inpdata + i * 3]
                                                - input[inpdata - natoms * 3 + i * 3]);
                        delta[1]  = positive_int(input[inpdata + i * 3 + 1]
                                                - input[inpdata - natoms * 3 + i * 3 + 1]);
                        delta[2]  = positive_int(input[inpdata + i * 3 + 2]
                ...
                else
                {
                    if ((new_runlength != runlength) || (new_small_index != small_index))
                    {
                            ...
                            for (ixx = 0; ixx < new_runlength; ixx++)
                            {
                                ...
                                do
                                {
                                    ...
                                    fprintf(stderr, "Tested decrease %d of index: %g>=%g?\n", change, isum,
                                            (double)Ptngc_magic(small_index + change)
                                                    * (double)Ptngc_magic(small_index + change));
#endif
                                    if (isum > (double)Ptngc_magic(small_index + change)
                                                       * (double)Ptngc_magic(small_index + change))
                                    {
#ifdef SHOWIT
                                        fprintf(stderr,
                        ...
                        if (runlength != new_runlength)
                        {
                            ...
                            insert_value_in_array(&xtc3_context.instructions, &xtc3_context.ninstr,
                                                  &xtc3_context.ninstr_alloc, INSTR_SMALL_RUNLENGTH,
                                                  "instr");
                            insert_value_in_array(&xtc3_context.rle, &xtc3_context.nrle,
                                                  &xtc3_context.nrle_alloc, (unsigned int)runlength,
                                                  "rle (small)");
                        }
#ifdef SHOWIT
                        fprintf(stderr, "Current small index: %d Base=%d\n", small_index,
                                Ptngc_magic(small_index));
#endif
                    }
...
#endif
                            insert_value_in_array(&xtc3_context.instructions, &xtc3_context.ninstr,
                                                  &xtc3_context.ninstr_alloc, INSTR_ONLY_SMALL,
                                                  "instr");
                        }
                        else
                        {
...
#ifdef SHOWIT
                            fprintf(stderr,
                                    "Sequence of one large and small integers (good "
                                    "compression).\n");
#endif
                            ...
                            insert_value_in_array(&xtc3_context.instructions, &xtc3_context.ninstr,
                                                  &xtc3_context.ninstr_alloc, INSTR_DEFAULT,
                                                  "instr");
...
#endif
                        insert_value_in_array(&xtc3_context.instructions, &xtc3_context.ninstr,
                                              &xtc3_context.ninstr_alloc, INSTR_ONLY_SMALL, "instr");
                    }
                    /* Insert the small integers into the small integer array. */
                    for (ienc = 0; ienc < runlength * 3; ienc++)
                    {
                        insert_value_in_array(&xtc3_context.smallintra, &xtc3_context.nsmallintra,
...
#    define bwlzh_compress_no_lz77 bwlzh_compress_no_lz77_verbose
    ...
    {
        if ((speed <= 2)
            || ((speed <= 5) && (!heuristic_bwlzh(xtc3_context.large_direct, xtc3_context.nlargedir))))
        {
            bwlzh_buf     = NULL;
        ...
        else
        {
            ...
            {
                bwlzh_compress_no_lz77(xtc3_context.large_direct, xtc3_context.nlargedir, bwlzh_buf,
                                       &bwlzh_buf_len);
            }
        }
    ...
    {
        if ((speed <= 2)
            || ((speed <= 5) && (!heuristic_bwlzh(xtc3_context.large_intra_delta, xtc3_context.nlargeintra))))
        {
            bwlzh_buf     = NULL;
            bwlzh_buf_len = INT_MAX;
        }
        else
        {
            ...
            if (speed >= 5)
            {
                bwlzh_compress(xtc3_context.large_intra_delta, xtc3_context.nlargeintra, bwlzh_buf,
                               &bwlzh_buf_len);
            }
            else
            {
                bwlzh_compress_no_lz77(xtc3_context.large_intra_delta, xtc3_context.nlargeintra,
                                       bwlzh_buf, &bwlzh_buf_len);
            }
        }
        /* If this can be written smaller using base compression we should do that. */
    ...
    {
        if ((speed <= 2)
            || ((speed <= 5) && (!heuristic_bwlzh(xtc3_context.large_inter_delta, xtc3_context.nlargeinter))))
        {
            bwlzh_buf     = NULL;
            bwlzh_buf_len = INT_MAX;
        }
        else
        {
            ...
            if (speed >= 5)
            {
                bwlzh_compress(xtc3_context.large_inter_delta, xtc3_context.nlargeinter, bwlzh_buf,
                               &bwlzh_buf_len);
            }
            else
            {
                bwlzh_compress_no_lz77(xtc3_context.large_inter_delta, xtc3_context.nlargeinter,
                                       bwlzh_buf, &bwlzh_buf_len);
            }
        }
        /* If this can be written smaller using base compression we should do that. */
    ...
    {
        if ((speed <= 2)
            || ((speed <= 5) && (!heuristic_bwlzh(xtc3_context.smallintra, xtc3_context.nsmallintra))))
        {
            bwlzh_buf     = NULL;
            bwlzh_buf_len = INT_MAX;
        }
        else
        {
            ...
            {
                bwlzh_compress_no_lz77(xtc3_context.smallintra, xtc3_context.nsmallintra, bwlzh_buf,
                                       &bwlzh_buf_len);
            }
        }
        /* If this can be written smaller using base compression we should do that. */
...
static void decompress_base_block(unsigned char** ptr, const int nvals, unsigned int** vals)
{
    int base_buf_len = (int)(((unsigned int)(*ptr)[0]) | (((unsigned int)(*ptr)[1]) << 8)
                             | (((unsigned int)(*ptr)[2]) << 16) | (((unsigned int)(*ptr)[3]) << 24));
    (*ptr) += 4;
    *vals = warnmalloc(nvals * sizeof(**vals));
                             ...
                             const int            current_large_type)
{
    ...
    else if (current_large_type == 1 && xtc3_context->large_intra_delta)
    {
        ...
        large_ints[1] =
                unpositive_int(xtc3_context->large_intra_delta[(*ilargeintra) + 1]) + prevcoord[1];
        large_ints[2] =
                unpositive_int(xtc3_context->large_intra_delta[(*ilargeintra) + 2]) + prevcoord[2];
        (*ilargeintra) += 3;
    }
...
int Ptngc_unpack_array_xtc3(unsigned char* packed, int* output, const int length, const int natoms)
{
    ...
    for (i = 0; i < 3; i++)
    {
        minint[i] = unpositive_int((int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                         | (((unsigned int)ptr[2]) << 16)
                                         | (((unsigned int)ptr[3]) << 24)));
        ptr += 4;
    }

    xtc3_context.ninstr = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.ninstr)
    {
    ...
    xtc3_context.nrle = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                              | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.nrle)
    {
    ...
    xtc3_context.nlargedir = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                   | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.nlargedir)
    {
    ...
    xtc3_context.nlargeintra = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                     | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.nlargeintra)
    {
    ...
    xtc3_context.nlargeinter = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                     | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.nlargeinter)
    {
    ...
    xtc3_context.nsmallintra = (int)(((unsigned int)ptr[0]) | (((unsigned int)ptr[1]) << 8)
                                     | (((unsigned int)ptr[2]) << 16) | (((unsigned int)ptr[3]) << 24));
    ptr += 4;
    if (xtc3_context.nsmallintra)
    {
    ...
    while (ntriplets_left > 0 && iinstr < xtc3_context.ninstr)
    {
            ...
            || (instr == INSTR_ONLY_SMALL)) /* only small */
        {
            if (instr != INSTR_ONLY_SMALL)
            {
                ...
                unpack_one_large(&xtc3_context, &ilargedir, &ilargeintra, &ilargeinter, prevcoord,
                                 minint, output, outdata, didswap, natoms, current_large_type);
                ntriplets_left--;
                outdata += 3;
            }
            if (instr != INSTR_ONLY_LARGE)
            {
                ...
                if ((instr == INSTR_DEFAULT) && (swapatoms))
                {
                    ...
                    for (i = 0; i < 3; i++)
                        fprintf(stderr, "%d %d %d\n", output[outdata - 3 + i * 3],
                                output[outdata - 2 + i * 3], output[outdata - 1 + i * 3]);
#endif
                }
                ntriplets_left -= runlength;
                outdata += runlength * 3;
        ...
        else if (instr == INSTR_LARGE_RLE && irle < xtc3_context.nrle)
        {
            ...
            {
                unpack_one_large(&xtc3_context, &ilargedir, &ilargeintra, &ilargeinter, prevcoord,
                                 minint, output, outdata, 0, natoms, current_large_type);
                ntriplets_left--;
                outdata += 3;
            }
        }
